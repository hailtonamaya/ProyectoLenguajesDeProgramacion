#include "Evaluador.h"

int main() {
    std::string expresionInfija = "10 + 5 * (2 - 7) / (3 + 2) * 4 / a";
    std::cout << "Expresion infija: " << expresionInfija << std::endl;

    if(evaluarInfija(expresionInfija)){
        std::cout << "Expresion infija evaluada correctamente" << std::endl;
        std::string expresionPostfija = infijaAPostfija(expresionInfija);
        std::cout << "Expresion postfija: " << expresionPostfija << std::endl;
        double resultadoInfija = evaluarPostfija(expresionPostfija);
        std::cout << "Resultado: " << resultadoInfija << std::endl;
    } else {
        std::cout << "Expresion infija no se pudo evaluar" << std::endl;
    }
    return 0;
}